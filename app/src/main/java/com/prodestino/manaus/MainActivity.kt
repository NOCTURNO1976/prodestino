...
class MainActivity : AppCompatActivity() {
    ...
    /** BASE_URL normalizada com barra final (evita redirect extra e problemas de cookie no 1º request) */
    private fun normalizedBase(): String {
        val base = BuildConfig.BASE_URL.ifBlank { "https://manaus.prodestino.com" }
        return base.trimEnd('/') + "/"
    }

    // Debounce para sair do offline apenas 1 vez a cada janela
    private var lastRecoverAt = 0L
    private fun recoverFromOfflineOnce() {
        if (!isOfflineAsset()) return
        val now = SystemClock.elapsedRealtime()
        if (now - lastRecoverAt < 3000L) return // 3s de guarda contra navegações concorrentes
        lastRecoverAt = now

        val startUrl = normalizedBase()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Listener de rede: se estiver em offline.html e a internet voltar, usa debounce para recuperar
        try {
    ...
    override fun onResume() {
        ...
        // IMPORTANTE: não navega aqui para evitar colisão com onAvailable()
        // Se estiver offline, aguardamos o callback de rede com debounce.
    }
    ...
    override fun onStop() {
        super.onStop()
        // Garante persistência dos cookies (reduz chance de “perder sessão” no 1º request pós-offline)

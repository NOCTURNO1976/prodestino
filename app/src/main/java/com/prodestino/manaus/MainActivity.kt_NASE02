...
class MainActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        // **App visível** → esconder a bolha (se o serviço estiver rodando)
    ...
    override fun onStop() {
        ...
        // **App em 2º plano** → mostrar a bolha (se o usuário já concedeu a permissão)
        if (hasOverlayPermission()) {
            startOverlayService(OverlayService.ACTION_SHOW)
        } else {
            // Pede ao usuário apenas quando fizer sentido
